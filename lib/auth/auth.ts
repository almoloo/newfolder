	...
	databaseHooks: {
		user: {
			create: {
				...
				after: async (user) => {
					// Grant every new user 10,000 stars (10,000 × 10^12 neuron) as initial credit
					const initialGrant = '10000000000000000';
					...
					await db.insert(schema.creditTransaction).values({
						...
						description:
							'Initial ★ 10,000 credit grant on registration',
					});
				},
			},
			update: {
